...
class SpikeDetector:
    def __init__(
        ...
        greeting_head_seconds: float = 0.0,
        greeting_tail_seconds: float = 0.0,
    ):
        self.min_prominence = min_prominence
        self.min_gap_seconds = min_gap_seconds
        self.pre_start_buffer_seconds = max(pre_start_buffer_seconds, 0.0)
        self.greeting_head_seconds = max(greeting_head_seconds, 0.0)
        self.greeting_tail_seconds = max(greeting_tail_seconds, 0.0)
    ...
    def detect(self, time_axis: np.ndarray, smoothed_series: np.ndarray) -> List[Spike]:
        ...
        baseline_series = self._select_baseline_window(time_axis, smoothed_series)
        baseline = np.mean(baseline_series)
    ...
    def _select_baseline_window(
        self, time_axis: np.ndarray, series: np.ndarray
    ) -> np.ndarray:
        if time_axis.size == 0:
            return series
        mask = np.ones_like(time_axis, dtype=bool)
        if self.greeting_head_seconds > 0:
            mask &= time_axis >= self.greeting_head_seconds
        if self.greeting_tail_seconds > 0:
            end_time = float(time_axis[-1])
            mask &= time_axis <= end_time - self.greeting_tail_seconds
        if not np.any(mask):
            return series
        return series[mask]
