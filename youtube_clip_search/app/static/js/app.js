...
function renderOriginalPreview() {
  ...
  originPreview.innerHTML = `
    <a class="origin-link" href="${currentOriginal.url}" target="_blank" rel="noopener">
      ${renderOriginThumbnail(thumbnail, altText)}
...
function renderOriginThumbnail(thumbnailUrl, altText) {
  if (thumbnailUrl) {
    return `
      <div class="origin-thumb">
        <img src="${thumbnailUrl}" alt="${escapeHtml(altText)}" loading="lazy" />
      </div>
    `;
  }
  const fallbackText = locale === "en" ? "No Image" : "画像なし";
  return `
    <div class="origin-thumb">
      <div class="origin-thumb-fallback">${fallbackText}</div>
    </div>
  `;
}
